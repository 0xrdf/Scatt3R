...
func TemplateTextReplace(ParamOne string, ParamTwo string, ParamThree string, ParamFour string, ParamFive string) {
	input, err := ioutil.ReadFile("client.go")
	...
	output = bytes.Replace(output, []byte("FileNameScatt3r"), []byte(string(ParamThree)), -1)
...
func ShowMenu() {
	ClearScreen()
	fmt.Println(CYAN, " Scatt3R ")
	fmt.Println(GREEN, "                                                Version: "+VERSION)
	fmt.Println("################ sysEv1L  ################")
	fmt.Println(YELLOW, " [*] type a number for the menu below [*]")
	fmt.Println(MAGENTA, "[*] [1] Generate Payload ")
	fmt.Println(MAGENTA, "[*] [2] Listen")
	fmt.Println(MAGENTA, "[*] [3] Serve")
	fmt.Println(RED, "[*] [4] Quit")
	OPT := ReadLine()
...
func GenerateCode() {
	ClearScreen()
	fmt.Println("")
	fmt.Println(GREEN, "GENERATE PAYLOAD")
	fmt.Println("")
